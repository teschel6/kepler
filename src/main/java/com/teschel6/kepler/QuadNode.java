 ...
 * and also: "A hierarchical O(N log N) force-calculation algorithm" by Barnes &
 * Hut
 */
    ...
    void insert(Particle p) {
        ...
        if (empty) {
            ...
            if (p.getX() < x + w) {
                if (p.getY() < y + w) {
                    if (nw == null)
                        nw = new QuadNode(x, y, w / 2);
                    nw.insert(p);
                } else {
                    if (sw == null)
                        sw = new QuadNode(x, y + w, w / 2);
                    sw.insert(p);
                }
            } else {
                if (p.getY() < y + w) {
                    if (ne == null)
                        ne = new QuadNode(x + w, y, w / 2);
                    ne.insert(p);
                } else {
                    if (se == null)
                        se = new QuadNode(x + w, y + w, w / 2);
                    se.insert(p);
                }
            }
